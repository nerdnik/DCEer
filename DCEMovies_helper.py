...
def prep_save_worms_double():
    ...
    if ans == 'y':
        dir_a = 'saved_worms/double/a/'
        for f in os.listdir(dir_a):
            os.remove(dir_a + f)

        dir_b = 'saved_worms/double/b/'
        for f in os.listdir(dir_b):
            os.remove(dir_b + f)

        open('saved_worms/double/a/WORM_INFO.txt', 'w').close()
        open('saved_worms/double/b/WORM_INFO.txt', 'w').close()
    else:
        sys.exit()
...
def save_worms_double(filename1, filename2, i, tau1, tau2, embed_crop):
    ...
    out_filename1 = 'saved_worms/double/a/' + filename1.split('/')[-1]
    out_filename2 = 'saved_worms/double/b/' + filename2.split('/')[-1]

    shutil.copyfile('data/embedded_coords_comp1.txt', out_filename1)
    shutil.copyfile('data/embedded_coords_comp2.txt', out_filename2)

    info_file_1 = open('saved_worms/double/a/WORM_INFO.txt', 'a')
    info_file_1.write('{:d}: tau = {} (samples), embed_crop = [{:.5f}, {:.5f}] (seconds) \n'.format(i, tau1, embed_crop[0], embed_crop[1]))
    info_file_1.close()

    info_file_1 = open('saved_worms/double/b/WORM_INFO.txt', 'a')
...
def prep_save_worms_single():
    ans = raw_input("Old worms will be deleted. OK? (y/n) \n")
    if ans == 'y':
        dir = 'saved_worms/single/'
        for f in os.listdir(dir):
            os.remove(dir + f)

        open('saved_worms/single/WORM_INFO.txt', 'w').close()
    else:
        sys.exit()
...
def save_worms_single(filename, i, tau, embed_crop):
    ...
    out_filename = 'saved_worms/single/' + str(i) + '-' + filename.split('/')[-1]
    shutil.copyfile('data/embedded_coords.txt', out_filename)
    info_file_1 = open('saved_worms/single/WORM_INFO.txt', 'a')
